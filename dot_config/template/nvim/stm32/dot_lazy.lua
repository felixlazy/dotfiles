return {
		...
		opts = function()
			...
			require("dap").configurations.c = {
				dap_cortex_debug.openocd_config({
					name = "STM32 DAP",
					cwd = "${workspaceFolder}",
					executable = "${workspaceFolder}/build/bln750_pwrv.elf",
					configFiles = {
	...
	},
	{
		...
		opts = function()
			...
			local function find_cfg()
				local files = vim.fn.glob(root() .. "/**/*.cfg", false, true)
				return files[1]
			end
