	...
	_migrate = function(self)
		...
		if oldConfig and not newConfig then
			...
			if dirExists and not utils.readdir(configDir) then
				log.warn(
					('Configuration migration failed. "%s" exists and does not appear to be a folder'):format(configDir)
				)
				return
			else
...
settings["default-style"] = [[\fnMaple Mono NF CN\b1\i1\bord2\shad0\fs30\c&Hf5d3c8&\3c&H2D2D2D&]]
		...
		redraw = function(self)
			...
			if position ~= self.lastPosition or self.needsUpdate then
				...
				if self.barShift > 0 then
					...
					self.line[7] = ([[\clip(m %g 0 l %g 0 %g %g %g %g)]]):format(
						followingEdge,
						Window.w,
						Window.w,
						Window.h,
				...
				if ranges and (#ranges > 0) then
					...
					for _index_0 = 1, #ranges do
						...
						elseif rangeStart > progressPosition then
							...
							local rectPast = ("m %g 0 l %g 1 %g 1 %g 0"):format(
								rangeStart,
								rangeStart,
								progressPosition,
								progressPosition
							)
							local rectFuture = ("m %g 0 l %g 1 %g 1 %g 0"):format(
								progressPosition,
								progressPosition,
								rangeEnd,
								rangeEnd
...
do
	...
	local _base_0 = {
		createMarkers = function(self)
			...
			local markerHeight = self.active and maxHeight * maxHeightFrac
				or BarBase.instantiatedBars[1].animationMinHeight
...
do
	...
	local _base_0 = {
		...
		resize = function(self)
			_class_0.__parent.__base.resize(self)
			self.line[2] = ("%g,%g"):format(
				clamp(Mouse.x, leftMargin, Window.w - rightMargin),
		...
		redraw = function(self)
			if self.active then
				...
				if Mouse.x ~= self.lastX and not self.thumbfast.disabled then
					...
					local thumbX =
						clamp(self.lastX, leftMargin + (scaledWidth / 2), Window.w - rightMargin - (scaledWidth / 2))
					...
					mp.commandv(
						...
						hoverTime,
						clamp(
							Mouse._rawX - self.thumbfast.width / 2,
